...
- **Export and share** — **File → Export as PDF…** saves the rendered document as one continuous page; **Export as HTML…** writes a fully self-contained single file (styles inlined, local images embedded); the Share button sends the document (or a PR link) to Mail, Messages, AirDrop, and friends.
...
- **Margin notes** *(beta, on by default)* — comment on any local Markdown document the way you would on a PR: hover a block, click the bubble (select text first to quote it), write Markdown. Notes save into the file itself as `<!-- note @you: … -->` HTML comments that stay out of the rendered Markdown — PullMark shows them as signed cards pinned to their spot — so handing feedback to an agent is just "address the notes in the file and delete them as you go." Open Files rows show a note-count chip; the [format is documented](https://pullmark.app/docs/experimental/margin-notes/) so agents can read and write it, and [pullmark.app/agents](https://pullmark.app/agents/) walks the whole loop.
