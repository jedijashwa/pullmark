...
- **Margin notes** *(beta)* — comment on any local Markdown document the way you would on a PR: hover a block, click the bubble (select text first to quote it), write Markdown. Notes save into the file itself as `<!-- note @you: … -->` HTML comments — invisible everywhere else, rendered by PullMark as bubbles pinned to their spot — so handing them to an agent is just "address the notes in the file and delete them as you go." Open Files rows show a note-count chip; the [format is documented](https://pullmark.app/docs/beta/margin-notes/) so agents can read and write it. Ships on the [beta channel](https://pullmark.app/docs/beta/): `brew install --cask jedijashwa/tap/pullmark@beta`.
