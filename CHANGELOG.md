...
- **Rich editor (alpha).** Settings › Experimental › Rich Editor (after
  Show alpha features) makes
  ...
  ⌘S, per the new Save edits setting. Alpha means what the Experimental
  tab says: it may change incompatibly or be replaced. Off by default;
  the section editor is untouched until you switch.
...
- **Built for macOS 27 Golden Gate.** PullMark now ships against the
  macOS 27 SDK, so on Golden Gate the window takes the new design the
  way Safari and Preview do: a uniform toolbar with grouped capsule
  items, the sidebar edge to edge with its divider running to the top,
  and inactive windows that dim like theirs. Back and Forward share
  their capsule with a hairline between them, and the Outline button
  shows when the outline is open, as Blame and Edit already did. The
  same build keeps its Liquid Glass toolbar on macOS 26 and still runs
...
- Fixed a crash when collapsing a folder in the sidebar after files had
  been added under one of its expanded subfolders that hadn't been
  scrolled into view — the everyday shape being a restored session in a
  growing repository, where the first collapse of the root took the app
  down.
- The session (open files, folders and their expansion, pull requests,
  browsed repos) is now saved a second after every change, not only at
  quit, so a crash or force-quit no longer restores a session from days
  ago.
