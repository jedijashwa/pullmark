...
enum ReadingPositions {
    ...
    static func save(_ fraction: Double, for key: String) {
        var all = UserDefaults.pullmark.dictionary(forKey: DefaultsKeys.readingPositions)
            as? [String: Double] ?? [:]
        ...
        UserDefaults.pullmark.set(all, forKey: DefaultsKeys.readingPositions)
    }

    static func fraction(for key: String) -> Double? {
        (UserDefaults.pullmark.dictionary(forKey: DefaultsKeys.readingPositions)
